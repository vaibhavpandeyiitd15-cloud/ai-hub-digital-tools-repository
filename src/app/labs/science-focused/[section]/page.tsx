...
export async function generateMetadata({ params }: PageProps) {
  ...
  const config = getScienceSection(section);
  return {
    title: config
      ? `${config.name} | Science Focused Lab | ${SITE_NAME}`
      : `Science Focused Lab | ${SITE_NAME}`,
  };
}

export default async function ScienceSectionPage({ params }: PageProps) {
  const { section } = await params;
  const config = getScienceSection(section);
  if (!config) notFound();
  ...
  return (
    <div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        <LabBreadcrumbs
          items={breadcrumbs(
            { label: "Science Focused Lab", href: "/labs/science-focused" },
            { label: config.name },
          )}
        />
